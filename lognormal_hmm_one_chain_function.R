...
# inits$mean0, mean1, mean2, disps0, disps1, disps2, mean.prior.scale, disp.prior.scale; 
# as well as initial values for parameters estimated by GS for input to first iteration 
# of MH for other parameters: mean.prior.location, disp.prior.location
...
ln_hmm_1_chain <- function(counts, groups, chain.length, thin=1, inits, 
  ...
  for (iter in 1:chain.length) {
    ...
    current.posterior.mean.prior.location <- 
      rnorm(n=1, 
            mean=prior.location.posterior.mean(hyperprior.mean=2.5, 
                                               hyperprior.var=20, 
                                               prior.scale=current.posterior.mean.prior.scale, 
    ...
    current.posterior.disp.prior.location <- 
      rnorm(n=1, 
            mean=prior.location.posterior.mean(hyperprior.mean=-2.5, 
                                               hyperprior.var=2, 
                                               prior.scale=current.posterior.disp.prior.scale, 
