...
for (i in 1:50) {
  ...
  filename <- paste0('disp.results.', filename, '.DESeqnorm.rds')
  saveRDS(results, file=here(filename))
  
  rm(list=c('counts', 'nf', 'norm', 'dge', 
            'dat.edgeR', 'dat.DESeq', 'notrend.DSS', 'trend.DSS',  'dat.DSS', 
            'filename', 'results'))
}

filename <- paste0('sessionInfo.dispersion.nodiff', samples.per.cond, '.DESeqnorm.rds')
