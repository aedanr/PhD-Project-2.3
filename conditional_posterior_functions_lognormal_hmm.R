...
mean.conditional.log.posterior <- function(n, sample.means, means, disps, 
...
disp.conditional.log.posterior = function(genes, counts, n, sample.means, means, disps, 
                                           prior.location, prior.scale) {
  out <- rep(-1e20,genes)
  index <- which(disps>0)
...
# Uses vector of posterior indicators
...
# Uses vectors of mean or dispersion estimates and posterior indicators
prior.location.posterior.mean <- function(hyperprior.mean, hyperprior.var, prior.scale, 
                                           parameters0, parameters1, parameters2, z) {
  return((hyperprior.mean / hyperprior.var + 
...
# Uses vectors of per-gene mean estimates and mixture components
mean.prior.scale.log.posterior <- function(prior.scale, prior.location, g, 
                                            means0, means1, means2, z) {
  if (prior.scale <= 0) {
    return(-1e20)
  } else {
    return((2 - (g + sum(z))/2) * log(prior.scale) - 
             0.8*prior.scale - 
             sum((1-z)*(log(means0) - prior.location)^2 + 
                   z*((log(means1) - prior.location)^2 + (log(means2) - prior.location)^2)) /
             (2*prior.scale))
  }
}
...
# Uses vectors of per-gene dispersion estimates and mixture components
disp.prior.scale.log.posterior <- function(prior.scale, prior.location, g, 
                                            disps0, disps1, disps2, z) {
  if (prior.scale < 0) {
    return(-1e20)
  } else {
    return((4 - (g + sum(z))/2) * log(prior.scale) - 
             2*prior.scale - 
             sum((1-z)*(log(disps0) - prior.location)^2 + 
                   z*((log(disps1) - prior.location)^2 + 
                        (log(disps2) - prior.location)^2)) / (2*prior.scale))
  }
}
...
pz0 <- function(genes, counts, n, sample.means, means, disps, 
                mean.prior.location, mean.prior.scale, 
