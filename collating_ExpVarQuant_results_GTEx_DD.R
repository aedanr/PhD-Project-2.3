...
for (samples.per.cond in c(2,5,10,20,50)) {
  ...
  for (i in 1:10) {
    ...
    filename <- paste0("results.ExpVarQuantmuscle_", samples.per.cond, "_set", i, "_DD.rds")
  ...
  saveRDS(results, file=here(folder, paste0("DD.results.muscle_", samples.per.cond, "_DD.rds")))
