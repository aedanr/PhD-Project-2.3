...
folder <- paste0("Simulated data")
# folder <- paste0("Data/Simulated data")
...
for (samples.per.cond in c(2,5,10,20,50)) {
  ...
  for (i in 1:50) {
    ...
    saveRDS(results, here("Results/ExpVarQuant compcodeR results Dec 2020", paste0("results.ExpVarQuant", filename, ".rds")))
    # saveRDS(results, here("Data/ExpVarQuant compcodeR results Dec 2020", paste0("results.ExpVarQuant", filename, ".rds")))
