...
for (i in 1:10) {
  ...
  filename <- paste0("muscle_", samples.per.cond, "_set", i, "_DE")
  ...
  #                               normalizationFactor=sizeFactors(dat.DESeq2.rle))
  ...
  rm(fit.edgeR.ql.tmm, test.edgeR.ql.tmm, fit.edgeR.lr.tmm, test.edgeR.lr.tmm, edgeR.et.tmm)
  ...
  rm(counts, DE, DEDD, FC, libsizes, 
     nf.tmm, els.tmm, sf.tmm, norm.counts.tmm, 
     nf.rle, els.rle, sf.rle, norm.counts.rle)

}

filename <- paste0("sessionInfo.GTEx_muscle_", samples.per.cond, "_set", i, "_DE.rds")
