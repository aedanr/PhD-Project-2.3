# Need to load log conditional posterior functions and base LN HM function
# Inputs: 
...
ln_hmm_adapt_3_chains <- function(counts, groups, chain.length=2000, 
  ...
  current.mean.prior.scale.proposal.sd <- 
    adapt.mcmc$mean.prior.scale.proposal.sd*(1+2.25*(adapt.mcmc$accept.mean.prior.scale-0.44))
  current.disp.prior.scale.proposal.sd <- 
    adapt.mcmc$disp.prior.scale.proposal.sd*(1+2.25*(adapt.mcmc$accept.disp.prior.scale-0.44))
  adapt.mcmc <- ln_hmm_3_chains(counts=counts, groups=groups, 
