...
#            'disp.diff.lnHM', 'log.disp.diff.lnHM'))
  rm(list=c('lnHM', 'log.mean.diff.lnHM'))
  ...
  filename <- paste0('results.', filename, '.lfc1.lnHM_lfc2.lnHM.rds')
  saveRDS(results, file=here(filename))

#  rm(list=c('counts', 'DE', 'lfcm1', 'lfcm2', 'nf', 'norm', 'dge', 'filename', 'results'))
  rm(list=c('counts', 'DE', 'lfcm1', 'lfcm2', 'nf', 'norm', 'filename', 'results'))
}

filename <- paste0('sessionInfo.DE', samples.per.cond, '.lfc1.lnHM_lfc2.lnHM.rds')
