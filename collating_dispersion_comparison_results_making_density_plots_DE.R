...
for (n in 1:50) {
  for (i in c('.DE2', '.DE5', '.DE10', '.DE20')) {
    res <- readRDS(here('Results/Dispersion estimation results Aug-Oct 2019', 
                        paste0('disp.results', i, '.', n, '.DESeqnorm.rds')))
...
saveRDS(DE2, file=here('Results/Dispersion estimation results Aug-Oct 2019', 
                       'mse.disp.DE2.DESeqnorm.rds'))
saveRDS(DE5, file=here('Results/Dispersion estimation results Aug-Oct 2019', 
                       'mse.disp.DE5.DESeqnorm.rds'))
saveRDS(DE10, file=here('Results/Dispersion estimation results Aug-Oct 2019', 
                        'mse.disp.DE10.DESeqnorm.rds'))
saveRDS(DE20, file=here('Results/Dispersion estimation results Aug-Oct 2019', 
                        'mse.disp.DE20.DESeqnorm.rds'))
...
plot(log(edgeR.tag[which(DE == 0)]), log(lnHM.overall_est[which(DE == 0)]), 
     pch=20, xlim=c(-3,2), ylim=c(-5,2))
# lnHM estimates v edgeR tag estimates on log scale for non-DE genes
plot(log(edgeR.tag[which(DE == 1)]), log(lnHM.overall_est[which(DE == 1)]), 
     pch=20, xlim=c(-3,2), ylim=c(-5,2))
...
plot(true.disps[which(DE == 0)], xlim=c(0,8), ylim=c(-1,1), 
     abs(lnHM.overall_est - true.disps)[which(DE == 0)] - 
       abs(edgeR.tag - true.disps)[which(DE == 0)], pch=20)
...
plot(true.disps[which(DE == 1)], xlim=c(0,8), ylim=c(-1,1), 
     abs(lnHM.overall_est - true.disps)[which(DE == 1)] - 
       abs(edgeR.tag - true.disps)[which(DE == 1)], pch=20)
...
plot(density(log(edgeR.tag[which(DE == 0)])), col='red', ylim=c(0,0.7), xlim=c(-5,2))
lines(density(log(lnHM.overall_est[which(DE == 0)])), col='blue')
lines(density(log(true.disps[which(DE == 0)])))
abline(v=mean(log(true.disps[which(DE == 0)])))
median(edgeR.tag)
median(lnHM.overall_est)
median(true.disps)
plot(density(log(edgeR.tag[which(DE == 1)])), col='red', ylim=c(0,0.8), xlim=c(-5,2))
lines(density(log(lnHM.overall_est[which(DE == 1)])), col='blue')
lines(density(log(true.disps[which(DE == 1)])))
...
# This is for DESeq normalisation, and normalisation makes a huge difference. edgeR tag with 
# TMM is far, far closer to true distribution than lnHM with DESeq normalisation.
...
## TMM for edgeR, DESeq norm for others
## Get data for both normalisation methods for all methods though for density comparisons
for (i in c('.DE2', '.DE5', '.DE10', '.DE20')) {
  norm <- '.DESeqnorm'
  res <- readRDS(here('Results/Dispersion estimation results Aug-Oct 2019', 
                      paste0('disp.results', i, '.', '50.DESeqnorm.rds')))
  ...
  assign(paste0('mean.counts_DE1',i,norm) , 
         rowMeans(res$data@count.matrix[which(get(paste0('DE',i)) == 1), 
                                        which(res$data@sample.annotations$condition == 1)]))
  assign(paste0('mean.counts_DE2',i,norm) , 
         rowMeans(res$data@count.matrix[which(get(paste0('DE',i)) == 1), 
                                        which(res$data@sample.annotations$condition == 2)]))
  assign(paste0('mean.counts_noDE',i,norm) , 
         rowMeans(res$data@count.matrix[which(get(paste0('DE',i)) == 0),]))
  assign(paste0('DESeq2',i,norm), res$disps.DESeq)
  assign(paste0('DESeq2_DE',i,norm), res$disps.DESeq[which(get(paste0('DE',i)) == 1)])
  assign(paste0('DESeq2_noDE',i,norm), res$disps.DESeq[which(get(paste0('DE',i)) == 0)])
  assign(paste0('DSS.notrend',i,norm), res$disps.DSS.notrend)
  assign(paste0('DSS.notrend_DE',i,norm), 
         res$disps.DSS.notrend[which(get(paste0('DE',i)) == 1)])
  assign(paste0('DSS.notrend_noDE',i,norm), 
         res$disps.DSS.notrend[which(get(paste0('DE',i)) == 0)])
  assign(paste0('DSS.trend',i,norm), res$disps.DSS.trend)
  assign(paste0('DSS.trend_DE',i,norm), 
         res$disps.DSS.trend[which(get(paste0('DE',i)) == 1)])
  assign(paste0('DSS.trend_noDE',i,norm), 
         res$disps.DSS.trend[which(get(paste0('DE',i)) == 0)])
  assign(paste0('edgeR.trend',i,norm), res$disps.edgeR.trended)
  assign(paste0('edgeR.trend_DE',i,norm), 
         res$disps.edgeR.trended[which(get(paste0('DE',i)) == 1)])
  assign(paste0('edgeR.trend_noDE',i,norm), 
         res$disps.edgeR.trended[which(get(paste0('DE',i)) == 0)])
  assign(paste0('edgeR.tag',i,norm), res$disps.edgeR.tagwise)
  assign(paste0('edgeR.tag_DE',i,norm), 
         res$disps.edgeR.tagwise[which(get(paste0('DE',i)) == 1)])
  assign(paste0('edgeR.tag_noDE',i,norm), 
         res$disps.edgeR.tagwise[which(get(paste0('DE',i)) == 0)])
  assign(paste0('expHM.group1_est',i,norm), res$disps.expHM.1)
  assign(paste0('expHM.group1_est_DE',i,norm), 
         res$disps.expHM.1[which(get(paste0('DE',i)) == 1)])
  assign(paste0('expHM.group1_est_noDE',i,norm), 
         res$disps.expHM.1[which(get(paste0('DE',i)) == 0)])
  assign(paste0('expHM.group2_est',i,norm), res$disps.expHM.2)
  assign(paste0('expHM.group2_est_DE',i,norm), 
         res$disps.expHM.2[which(get(paste0('DE',i)) == 1)])
  assign(paste0('expHM.group2_est_noDE',i,norm), 
         res$disps.expHM.2[which(get(paste0('DE',i)) == 0)])
  assign(paste0('expHM.overall_est',i,norm), res$disps.expHM)
  assign(paste0('expHM.overall_est_DE',i,norm), 
         res$disps.expHM[which(get(paste0('DE',i)) == 1)])
  assign(paste0('expHM.overall_est_noDE',i,norm), 
         res$disps.expHM[which(get(paste0('DE',i)) == 0)])
  assign(paste0('lnHM.group1_est',i,norm), res$disps.lnHM.1)
  assign(paste0('lnHM.group1_est_DE',i,norm), 
         res$disps.lnHM.1[which(get(paste0('DE',i)) == 1)])
  assign(paste0('lnHM.group1_est_noDE',i,norm), 
         res$disps.lnHM.1[which(get(paste0('DE',i)) == 0)])
  assign(paste0('lnHM.group2_est',i,norm), res$disps.lnHM.2)
  assign(paste0('lnHM.group2_est_DE',i,norm), 
         res$disps.lnHM.2[which(get(paste0('DE',i)) == 1)])
  assign(paste0('lnHM.group2_est_noDE',i,norm), 
         res$disps.lnHM.2[which(get(paste0('DE',i)) == 0)])
  assign(paste0('lnHM.overall_est',i,norm), res$disps.lnHM)
  assign(paste0('lnHM.overall_est_DE',i,norm), 
         res$disps.lnHM[which(get(paste0('DE',i)) == 1)])
  assign(paste0('lnHM.overall_est_noDE',i,norm), 
...
plot(log(true.disps_noDE.DE2), log(edgeR.tag_noDE.DE2.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='edgeR')
lines(c(-6,2), c(-6,2))
legend('topleft', legend='2 samples per group', bty='n', text.font=2)
plot(log(true.disps_noDE.DE2), log(DESeq2_noDE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='DESeq2')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE2), log(DSS.notrend_noDE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='DSS')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE2), log(lnHM.overall_est_noDE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='HM')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE5), log(edgeR.tag_noDE.DE5.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='5 samples per group', bty='n', text.font=2)
plot(log(true.disps_noDE.DE5), log(DESeq2_noDE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE5), log(DSS.notrend_noDE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE5), log(lnHM.overall_est_noDE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE10), log(edgeR.tag_noDE.DE10.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='10 samples per group', bty='n', text.font=2)
plot(log(true.disps_noDE.DE10), log(DESeq2_noDE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE10), log(DSS.notrend_noDE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE10), log(lnHM.overall_est_noDE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE20), log(edgeR.tag_noDE.DE20.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='20 samples per group', bty='n', text.font=2)
plot(log(true.disps_noDE.DE20), log(DESeq2_noDE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE20), log(DSS.notrend_noDE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_noDE.DE20), log(lnHM.overall_est_noDE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))

# DE genes
par(mfrow=c(4,4), mar=c(1.5,1.5,2,1), mgp=c(2,0.5,0))
plot(log(true.disps_DE.DE2), log(edgeR.tag_DE.DE2.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='edgeR')
lines(c(-6,2), c(-6,2))
legend('topleft', legend='2 samples per group', bty='n', text.font=2)
plot(log(true.disps_DE.DE2), log(DESeq2_DE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='DESeq2')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE2), log(DSS.notrend_DE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='DSS')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE2), log(lnHM.overall_est_DE.DE2.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2), main='HM')
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE5), log(edgeR.tag_DE.DE5.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='5 samples per group', bty='n', text.font=2)
plot(log(true.disps_DE.DE5), log(DESeq2_DE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE5), log(DSS.notrend_DE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE5), log(lnHM.overall_est_DE.DE5.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE10), log(edgeR.tag_DE.DE10.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='10 samples per group', bty='n', text.font=2)
plot(log(true.disps_DE.DE10), log(DESeq2_DE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE10), log(DSS.notrend_DE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE10), log(lnHM.overall_est_DE.DE10.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE20), log(edgeR.tag_DE.DE20.TMM), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
legend('topleft', legend='20 samples per group', bty='n', text.font=2)
plot(log(true.disps_DE.DE20), log(DESeq2_DE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE20), log(DSS.notrend_DE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))
plot(log(true.disps_DE.DE20), log(lnHM.overall_est_DE.DE20.DESeqnorm), 
     pch=20, cex=0.5, xlim=c(-6,2), ylim=c(-6,2))
lines(c(-6,2), c(-6,2))


plot(density(log(true.disps_noDE.DE2)), xlim=c(-5,2), ylim=c(0,2.3))
lines(density(log(edgeR.tag_noDE.DE2.TMM)), col='red')
lines(density(log(DESeq2_noDE.DE2.TMM)), col='orange')
lines(density(log(lnHM.overall_est_noDE.DE2.TMM)), col='blue')
plot(density(log(true.disps_DE.DE2)), xlim=c(-5,2), ylim=c(0,2.3))
lines(density(log(edgeR.tag_DE.DE2.TMM)), col='red')
lines(density(log(DESeq2_DE.DE2.TMM)), col='orange')
lines(density(log(lnHM.overall_est_DE.DE2.TMM)), col='blue')
plot(density(log(true.disps_noDE.DE2)), xlim=c(-5,2), ylim=c(0,2.3))
lines(density(log(edgeR.tag_noDE.DE2.DESeqnorm)), col='red')
lines(density(log(DESeq2_noDE.DE2.DESeqnorm)), col='orange')
lines(density(log(lnHM.overall_est_noDE.DE2.DESeqnorm)), col='blue')
plot(density(log(true.disps_DE.DE2)), xlim=c(-5,2), ylim=c(0,2.3))
lines(density(log(edgeR.tag_DE.DE2.DESeqnorm)), col='red')
lines(density(log(DESeq2_DE.DE2.DESeqnorm)), col='orange')
lines(density(log(lnHM.overall_est_DE.DE2.DESeqnorm)), col='blue')

par(mfrow=c(2,2), mar=c(1.5,1.5,2,1), mgp=c(2,0.5,0))
plot(density(log(true.disps_noDE.DE20)), xlim=c(-5,2), ylim=c(0,0.7), lwd=2, 
     main='Non-DE genes, TMM normalisation', yaxt='n')
legend("topleft", fill=c("black", "red", "orange", "blue"), bty="n", cex=1.2, 
       legend=c("True dispersions", "edgeR estimates", "DESeq2 estimates", "lnHM estimates"))
lines(density(log(edgeR.tag_noDE.DE20.TMM)), col='red', lwd=2)
lines(density(log(DESeq2_noDE.DE20.TMM)), col='orange', lwd=2)
lines(density(log(lnHM.overall_est_noDE.DE20.TMM)), col='blue', lwd=2)
plot(density(log(true.disps_DE.DE20)), xlim=c(-5,2), ylim=c(0,0.7), lwd=2, 
     main='DE genes, TMM normalisation', yaxt='n')
lines(density(log(edgeR.tag_DE.DE20.TMM)), col='red', lwd=2)
lines(density(log(DESeq2_DE.DE20.TMM)), col='orange', lwd=2)
lines(density(log(lnHM.overall_est_DE.DE20.TMM)), col='blue', lwd=2)
plot(density(log(true.disps_noDE.DE20)), xlim=c(-5,2), ylim=c(0,0.7), lwd=2, 
     main='Non-DE genes, DESeq normalisation', yaxt='n')
lines(density(log(edgeR.tag_noDE.DE20.DESeqnorm)), col='red', lwd=2)
lines(density(log(DESeq2_noDE.DE20.DESeqnorm)), col='orange', lwd=2)
lines(density(log(lnHM.overall_est_noDE.DE20.DESeqnorm)), col='blue', lwd=2)
plot(density(log(true.disps_DE.DE20)), xlim=c(-5,2), ylim=c(0,0.7), lwd=2, 
     main='DE genes, DESeq normalisation', yaxt='n')
lines(density(log(edgeR.tag_DE.DE20.DESeqnorm)), col='red', lwd=2)
lines(density(log(DESeq2_DE.DE20.DESeqnorm)), col='orange', lwd=2)
lines(density(log(lnHM.overall_est_DE.DE20.DESeqnorm)), col='blue', lwd=2)
