...
for (samples.per.cond in c(2, 5, 10, 20, 50)) {
  ...
  for (i in 1:50) {
    filename <- paste0("results.DD", samples.per.cond, ".", i, ".rds")
  ...
  saveRDS(results.DD, file=here(folder, paste0("DD.results.", "DD", samples.per.cond, ".rds")))
  saveRDS(results.DEDD, file=here(folder, paste0("DEDD.results.", "DD", samples.per.cond, ".rds")))
}
